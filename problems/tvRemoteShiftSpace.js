function tvRemoteShiftSpace(word) {
    ...
    for(let i = 1; i < word.length; i++) {
       ...
       if(isNumberOrSymbol(char)) {
          output = output + pressesBetweenChars(word[i - 1].toLowerCase(), char) + 1;
          continue;
       }

       if(isCapitalized(char)) {
          if(capLock) {
             output = output + pressesBetweenChars(word[i - 1].toLowerCase(), char.toLowerCase()) + 1
          } else {  
             output = output + pressesBetweenChars(word[i - 1].toLowerCase(), 'cap') + 1;
             capLock = true;
